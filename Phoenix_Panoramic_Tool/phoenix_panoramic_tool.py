...
def download_cameras(country_code, expiry_from, expiry_to, reported_from, reported_to, buffer_m=5.0):
    ...
    # Keep raw coordinates for panoramic spatial matching — clean format applied after
    df["coordinates"] = df.get("geometry.coordinates", "")
    ...
    # Clean coordinates — extract starting point only, remove brackets
    def _start_coord(val):
        try:
            import ast as _a
            v = _a.literal_eval(str(val)) if isinstance(val, str) else val
            ...
            pass
        return ""
    df["coordinates"] = df["coordinates"].apply(_start_coord)
