...
def validate_vertex_spacing(
    ...
    for lname, gdf in layers.items():
        ...
        feature_col = detect_column(gdf.columns, FEATURE_ID_CANDIDATES)
        ...
        for idx, geom in zip(work.index, work.geometry):
            feature_id_value = gdf.loc[idx, feature_col] if feature_col else None
            for ring_name, coords in _iter_polygon_rings(geom):
                for iss in _check_ring_vertex_spacing(coords, tolerance_m):
                    rows.append({
                        "source_layer": lname, "feature_id": feature_id_value, "row_index": idx,
                        "ring": ring_name, "vtx_idx": iss["vtx_idx"], "dist_m": iss["dist_m"],
                        "x": iss["x"], "y": iss["y"],
                    })

    flagged_df = pd.DataFrame(rows) if rows else pd.DataFrame(
        columns=["source_layer", "feature_id", "row_index", "ring", "vtx_idx", "dist_m", "x", "y"]
    )
        ...
        for idx, geom in zip(sub.index, sub.geometry):
            ...
            for ring_name, coords in _iter_polygon_rings(geom):
                for iss in _check_ring_spike_angles(coords, angle_deg):
                    rows.append({
                        "source_layer": lname, "feature_id": feature_id_value, "row_index": idx,
                        "ring": ring_name, "vtx_idx": iss["vtx_idx"], "angle_deg": iss["angle_deg"],
                        "x": iss["x"], "y": iss["y"],
                    })

    flagged_df = pd.DataFrame(rows) if rows else pd.DataFrame(
        columns=["source_layer", "feature_id", "row_index", "ring", "vtx_idx", "angle_deg", "x", "y"]
    )
...
def find_self_intersecting_polygons(
    ...
    for lname, gdf in layers.items():
        ...
        for idx, row in gdf.iterrows():
            ...
            # source_layer + Feature_Id lead every row so the flagged feature is identifiable at a glance.
            lead = {"source_layer": lname, "feature_id": row[feature_col] if feature_col else None}
            rest = {k: v for k, v in row.items() if k not in (geom_col, feature_col)}

            failed_row = {**lead, **rest}
    ...
    failed_df = pd.DataFrame(all_failed_rows) if all_failed_rows else pd.DataFrame(
        columns=["source_layer", "feature_id", "row_index", "issue", "intersection_point_count"]
    )
    points_df = pd.DataFrame(all_point_records) if all_point_records else pd.DataFrame(
        columns=["source_layer", "feature_id", "row_index", "pt_num", "x_coord", "y_coord", "issue"]
    )
